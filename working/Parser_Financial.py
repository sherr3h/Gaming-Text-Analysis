...
def price_variation(period,TICKER,movie):
    ...
    df_f = pd.read_csv("Financial_Data/%s.csv"%TICKER)
    ...
    average_volume = output_before["Volume"].mean(axis = 0,skipna = True)
    month_volatility = output_before["Close"].std(axis = 0,skipna = True)

    categorized_price_variation = 0

    if float(period_price_variation) > 0.02:
        categorized_price_variation = 1
    elif float(period_price_variation) < -0.02:
        categorized_price_variation = -1
    else:
        categorized_price_variation = 0

    c = [previous_month_price_variation,period_price_variation,average_volume,month_volatility,categorized_price_variation]
