...
def parser(days,TICKER):
    ...
    for movie in liste:
        ...
        print(movie) #ten times to notice it easily
        df = pd.read_csv("Raw_Data_Studios/DIS/%s"%movie,sep = ",")
        output = []
        sent_s = []
        sent_p = []
        ...
        if os.path.exists("Data_Studios/%s/Studio_%s_%s/%s.csv"%(TICKER,TICKER,days,movie)) == False:
...
def info(TICKER,movie):
    df = pd.read_csv("Raw_Data_Studios/%s/%s"%(TICKER,movie))
    date_ba = df.iloc[0,0].split('Z')[0]
    date_ba = datetime.strptime(date_ba, '%Y-%m-%dT%H:%M:%S.%f')
